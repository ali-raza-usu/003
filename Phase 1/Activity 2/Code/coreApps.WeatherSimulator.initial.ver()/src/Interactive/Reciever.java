...
public class Reciever extends Thread {
	...
	private Logger logger = Logger.getLogger(Reciever.class);
	...
	//public static SharedKey key=null;
	...
	public void coreReceiver(int port) throws IOException {
		try {
			...
			System.out.println("Remaing bytes in buffer : Receiver : are " + buffer.remaining());
			...
			while (keepRunning) {
				...
				if (readBuf.remaining() > 0) {
					...
					if (_message != null) {
						...
						if (addr != null && addr.toString().contains(port + "")) {
							isPktRcvd = true;
						...
						if(_message.getClass().getSimpleName().equals(WeatherDataVector.class.getSimpleName())){
							...
							for (WeatherDataReading _reading : _data.getReadings())
									logger.debug("Rcvd "+ " : Data Speed : " + _reading.getSpeed()+ " Temperature "+ _reading.getTemperature());
														
							writeDataToAFile(_data, "Rcvd from " + addr);
							_message = null;
							readBuf.clear();
						}
					}
...
//It is going to be rescheduled again & again, every time a packet is received
	...
	public static void main(String args[]) {
		...
		Reciever _receiver = new Reciever(portOne);
		_receiver.start();
	}
