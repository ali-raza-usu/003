...
public class Server extends Thread {
					...
					for (Iterator<SelectionKey> i = sckt_manager.selectedKeys()
						...
						if (key.isReadable()) {
								...
								;
							{
								...
								msg = (TranslationRequestMessage) convertBufferTorequestMessage(buffer);
							...
							int result = LevenshteinDistance(msg.getData1(),
									msg.getData2());
							TranslationResponseMessage Rmsg = new TranslationResponseMessage(
									"Levenshtein Distance between string : "
											+ msg.getData1() + " and string "
											+ msg.getData2() + " is " + result);
							buffer.clear();
							buffer = ByteBuffer.wrap(Encoder.encode(Rmsg));
							client.write(buffer);
							_logger.debug("Sending " + Rmsg.getResponse());
							if (msg.getData1().equals("quit")
	...
	private TranslationRequestMessage convertBufferTorequestMessage(ByteBuffer buffer) {
